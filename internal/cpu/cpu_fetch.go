...
func FetchData() {
	...
	case AM_IMP:
		...
		return
	case AM_R:
		// Fetches 8-bit value from Reg1. OK for INC/DEC, but for 16-bit INC/DEC, should not mask to 8 bits.
		// Fault: If Reg1 is 16-bit, this will not work as expected. See procInc/procDec for correct handling.
		cpuInstance.FetchedData = CpuRegRead(cpuInstance.currentInst.Reg1) & 0xFF
		return
	case AM_R_R:
		// Fetches 8-bit value from Reg2. OK for most LD r,r instructions.
	...
	case AM_HLD_R:
		...
		return
	case AM_R_A8:
		// LD A,($FF00+offset). FetchedData = [$FF00+offset].
		// Fault: Should mask offset to 8 bits (uint8), in case upper bits are set.
		...
		return
	case AM_A8_R:
		// LD ($FF00+offset),A. FetchedData = A, MemDest = $FF00+offset.
		// Fault: Should mask offset to 8 bits (uint8).
		offset := uint16(memory.BusCtx().BusRead(cpuInstance.Regs.Pc))
		Cm.IncreaseCycle(1)
		cpuInstance.Regs.Pc++
		cpuInstance.MemDest = 0xFF00 | (offset & 0xFF)
		cpuInstance.DestIsMem = true
		cpuInstance.FetchedData = CpuRegRead(cpuInstance.currentInst.Reg2) & 0xFF
		return
	case AM_HL_SPR:
